...
const ChatInterface = ({ control }) => {
  // Initialize messages with static messages
  const staticMessages = ["what are the top 5 suspicious transactions?"];
  ...
  // Load messages from localStorage for the current context
  useEffect(() => {
    const savedMessages = JSON.parse(localStorage.getItem(`${control}ChatHistory`) || "[]");
    const combinedMessages = [...staticMessages, ...savedMessages]; // Combine static and saved messages
    setMessages(combinedMessages); // Ensure it's an array
